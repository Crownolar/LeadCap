...
const CollectorPickerModal = ({ onClose }) => {
  ...
  const handleSelect = (collector = null) => {
    onClose();
    ...
    navigate(`/sample-review/${collector.id}`, {
      state: {
        collectorName:
          collector.fullName || collector.name || "Selected Collector",
      },
    });
  };
  ...
  return (
    ...
    >
      <div
        className={`relative w-full max-w-md rounded-3xl border shadow-2xl ${theme.card} ${theme.border}`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div
          className={`flex items-center justify-between border-b px-6 py-4 ${theme.border}`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-xs ${theme.textMuted}`}>
                Choose whose samples to review
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className={`rounded-xl p-2 transition hover:bg-gray-100 dark:hover:bg-gray-800 ${theme.textMuted}`}
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className={`border-b px-4 py-3 ${theme.border}`}>
          <div
            className={`flex items-center gap-2 rounded-2xl border px-3 py-2 ${theme.border} ${theme.card}`}
          >
            ...
            <input
              ...
              className={`flex-1 bg-transparent text-sm outline-none placeholder:text-gray-400 ${theme.text}`}
            />
          </div>
        </div>
        {/* List */}
        <div className="max-h-[340px] overflow-y-auto px-4 py-3 space-y-2">
          <button
            ...
            className={`w-full flex items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-left transition hover:-translate-y-[1px] hover:shadow-md ${theme.border} ${theme.card}`}
          >
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-500 text-white text-xs font-semibold">
                All
              </div>
              <div>
                <p className={`text-sm font-semibold ${theme.text}`}>
          ...
          {loading ? (
            ...
            <div className="py-10 text-center">
              <p className="text-sm font-medium text-red-600 dark:text-red-400">
                Unable to verify your collector scope.
              </p>
            ...
            filtered.map((collector) => {
              ...
              const sampleCount =
                collector.sampleCount ?? collector._count?.samples ?? null;
              return (  
                <button
                  key={collector.id}
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-emerald-700 text-xs font-semibold dark:bg-emerald-900/30 dark:text-emerald-300">
                      {getInitials(name)}
                    </div>
