...
const StatCard = ({ title, value, subtitle, icon, onClick }) => {
  ...
  const content = (
    <div className="flex items-start justify-between gap-4">
      <div>
        ...
        <p className="text-3xl font-bold tracking-tight text-emerald-600 dark:text-emerald-400 mt-1">{value}</p>
        {subtitle && <p className={`text-xs mt-2 ${theme.textMuted}`}>{subtitle}</p>}
      </div>
      {icon && (
  ...
  if (onClick) return <button onClick={onClick} className={`${base} text-left`}>{content}</button>;
