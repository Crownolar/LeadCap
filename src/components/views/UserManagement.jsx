...
const UserManagement = ({ theme, darkMode }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchUsers()
      setShowForm(false)
      setFormData({ fullName: '', email: '', role: 'DATA_COLLECTOR', organization: '', isActive: true })
      setEditingId(null)
    } catch (err) {
  ...
  return (
    <div className={`p-6 ${theme?.bg}`}>
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          <div className={`text-center text-lg ${theme?.text}`}>Loading users...</div>
        ) : filteredUsers.length === 0 ? (
          <div className={`text-center text-lg ${theme?.text}`}>No users found</div>
        ) : (
            ...
            <table className="w-full">
              <thead className={theme?.card}>
                <tr className={`border-b ${theme?.border}`}>
                  ...
                  <th className={`px-6 py-3 text-left ${theme?.text}`}>Email</th>
                  <th className={`px-6 py-3 text-left ${theme?.text}`}>Role</th>
                  <th className={`px-6 py-3 text-left ${theme?.text}`}>Organization</th>
                  <th className={`px-6 py-3 text-left ${theme?.text}`}>Status</th>
                  <th className={`px-6 py-3 text-center ${theme?.text}`}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredUsers.map((user) => (
                  <tr key={user.id} className={`border-b ${theme?.border} hover:${theme?.hover}`}>
                    <td className={`px-6 py-3 ${theme?.text}`}>{user.fullName}</td>
                    <td className={`px-6 py-3 ${theme?.textMuted}`}>{user.email}</td>
                    <td className={`px-6 py-3 ${theme?.text}`}>
                      <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                        {user.role}
                      </span>
                    </td>
                    <td className={`px-6 py-3 ${theme?.textMuted}`}>{user.organization || '-'}</td>
