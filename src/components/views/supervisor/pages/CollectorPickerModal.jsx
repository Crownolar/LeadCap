...
const CollectorPickerModal = ({ onClose }) => {
  ...
  const handleSelect = (collectorId) => {
    onClose();
    if (collectorId) {
      navigate(`/sample-review/${collectorId}`);
    } else {
      ...
      >
        ...
        <div className="max-h-[340px] overflow-y-auto px-4 py-3 space-y-2">
          ...
          {loading ? (
            ...
            <div className="py-10 text-center">
              <p className={`text-sm ${theme.textMuted}`}>No collectors found.</p>
            </div>
          ) : (
            filtered.map((collector) => {
              ...
              return (
                <button
                  key={collector.id}
                  onClick={() => handleSelect(collector.id)}
                  className={`w-full flex items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-left transition hover:-translate-y-[1px] hover:shadow-md ${theme.border} ${theme.card}`}
                >
                  <div className="flex items-center gap-3">
        ...
        <div className={`border-t px-6 py-3 ${theme.border}`}>
          <p className={`text-center text-xs ${theme.textMuted}`}>
            {collectors.length} collector{collectors.length !== 1 ? "s" : ""} assigned to you
          </p>
        </div>
      </div>
    </div>
