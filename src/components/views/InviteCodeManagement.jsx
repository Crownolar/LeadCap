...
const InviteCodeManagement = ({ theme, darkMode }) => {
  ...
  if (normalizedRole !== "superadmin") {
    return (
      <div className={`${displayTheme?.bg} min-h-screen flex items-center justify-center p-4`}>
        <div className={`${displayTheme?.card} rounded-lg border ${displayTheme?.border} shadow-md p-8 text-center max-w-md`}>
          <Lock className="w-16 h-16 mx-auto mb-4 text-yellow-600" />
          <h2 className={`${displayTheme?.text} text-2xl font-bold mb-2`}>Access Restricted</h2>
  ...
  const fetchInviteCodes = async () => {
    try {
      ...
      setError('Failed to fetch invite codes: ' + (err.response?.data?.error || err.message))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleGenerateInvite = async (e) => {
    ...
    try {
      const payload = {
        role: formData.role,
        ...(formData.role.startsWith('POLICY_MAKER') && { organization: formData.organization })
      }
      ...
      setError('Failed to generate invite code: ' + (err.response?.data?.error || err.message))
    }
  }
  ...
  const handleDeleteInvite = async (id) => {
    if (confirm('Are you sure you want to delete this invite code?')) {
      try {
        ...
        setError('Failed to delete invite code: ' + (err.response?.data?.error || err.message))
      }
    }
  }
  ...
  const filteredCodes = inviteCodes.filter(ic => {
    ...
    const matchesStatus = filterStatus === 'all' || (filterStatus === 'used' ? ic.isUsed : !ic.isUsed)
    ...
    <div className={`p-6 ${theme?.bg}`}>
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className={`text-3xl font-bold ${theme?.text}`}>Invite Code Management</h1>
          <button
        ...
        {loading ? (
          <div className={`text-center text-lg ${theme?.text}`}>Loading invite codes...</div>
        ) : filteredCodes.length === 0 ? (
          <div className={`text-center text-lg ${theme?.text}`}>No invite codes found</div>
        ) : (
            ...
            <table className="w-full">
              <thead className={theme?.card}>
                <tr className={`border-b ${theme?.border}`}>
                  ...
                  <th className={`px-6 py-3 text-left ${theme?.text}`}>Organization</th>
                  <th className={`px-6 py-3 text-center ${theme?.text}`}>Status</th>
                  <th className={`px-6 py-3 text-left ${theme?.text}`}>Generated By</th>
                  <th className={`px-6 py-3 text-left ${theme?.text}`}>Created</th>
                  <th className={`px-6 py-3 text-center ${theme?.text}`}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredCodes.map((ic) => (
                  <tr key={ic.id} className={`border-b ${theme?.border} hover:${theme?.hover}`}>
                    <td className={`px-6 py-3 ${theme?.text} font-mono text-sm`}>
                      <div className="flex items-center gap-2">
                        {ic.code}
                        <button
                          onClick={() => handleCopyCode(ic.code)}
                          className={`p-1 rounded ${copiedCode === ic.code ? 'bg-green-100' : theme?.hover}`}
                          title="Copy to clipboard"
                        >
                          <Copy size={16} className={copiedCode === ic.code ? 'text-green-600' : ''} />
                        </button>
                      </div>
                    </td>
                    ...
                    <td className={`px-6 py-3 ${theme?.textMuted}`}>{ic.organization || '-'}</td>
                    <td className={`px-6 py-3 text-center`}>
                      <span className={`px-3 py-1 rounded-full text-sm ${ic.isUsed ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                        {ic.isUsed ? 'Used' : 'Unused'}
                      </span>
                    </td>
                    <td className={`px-6 py-3 ${theme?.textMuted}`}>{ic.createdBy?.fullName || 'Unknown'}</td>
        ...
        <div className={`mt-6 p-4 ${theme?.card} border ${theme?.border} rounded-lg`}>
          ...
          <ol className={`space-y-1 ${theme?.textMuted} list-decimal list-inside`}>
            <li>Generate a unique invite code for the desired role</li>
            <li>Share the code with the person who needs to register</li>
            <li>They will enter this code during signup to get the specified role</li>
